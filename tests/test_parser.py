...
class SpsParserTests(unittest.TestCase):
    ...
    def test_instruction_in_event_description_is_parsed(self):
        ...
        result = parse_sps_instructions(source_shift(
            starts_at="2026-09-17T13:00:00+02:00", ends_at="2026-09-18T16:00:00+02:00",
            comment=None, notes="uni 12-14 fredag"), self.timezone)
        ...
        self.assertEqual("2026-09-18T12:00:00+02:00", result.intervals[0].interval.starts_at.isoformat())

    def test_description_and_comment_intervals_are_both_kept(self):
        result = parse_sps_instructions(source_shift(
            notes="uni 8-10", comment="uni 13-14"), self.timezone)
        self.assertEqual((), result.issues)
        self.assertEqual(
    ...
    def test_same_interval_in_description_and_comment_is_planned_once(self):
        result = parse_sps_instructions(source_shift(
            notes="uni 8-10", comment="uni 8-10"), self.timezone)
    ...
    def test_abbreviated_weekdays_resolve_like_full_names(self):
        for spelling in ("fre", "fre.", "FRE", "fredag"):
            with self.subTest(spelling=spelling):
                result = parse_sps_instructions(source_shift(
                    starts_at="2026-09-17T13:00:00+02:00", ends_at="2026-09-18T16:00:00+02:00",
                    comment=None, notes=f"uni 12-14 {spelling}"), self.timezone)
                self.assertEqual((), result.issues)
    ...
    def test_friday_suffix_resolves_second_day_of_september_shift(self):
        result = parse_sps_instructions(source_shift(
            starts_at="2026-09-17T13:00:00+02:00", ends_at="2026-09-18T16:00:00+02:00",
            comment="uni 12-14 fredag"), self.timezone)
        self.assertEqual((), result.issues)
        self.assertEqual("2026-09-18T12:00:00+02:00", result.intervals[0].interval.starts_at.isoformat())
    ...
    def test_weekday_suffix_applies_to_split_intervals(self):
        result = parse_sps_instructions(source_shift(
            starts_at="2026-09-17T13:00:00+02:00", ends_at="2026-09-18T16:00:00+02:00",
            comment="UNI 8-10 & 12-14 FREDAG"), self.timezone)
        self.assertEqual((), result.issues)
    ...
    def test_weekday_never_guesses_between_dates_or_overrides_bounds(self):
        for end, comment, code in (
            ...
            ("2026-09-18T16:00:00+02:00", "uni 12-14 lørdag", "uni_weekday_outside_shift"),
            ("2026-09-18T13:00:00+02:00", "uni 12-14 fredag", "uni_outside_shift"),
            ("2026-09-18T16:00:00+02:00", "uni 2026-09-17 14-15 fredag", "conflicting_uni_date"),
        ):
            with self.subTest(code=code):
                result = parse_sps_instructions(source_shift(starts_at="2026-09-17T13:00:00+02:00", ends_at=end, comment=comment), self.timezone)
